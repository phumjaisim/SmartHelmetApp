...
export const Button = ({ 
  ...
  const getButtonStyle = () => {
    ...
    return { gradient: styles[variant], size: sizes[size] };
  };

  const { gradient, size } = getButtonStyle();
  ...
  return (
    <TouchableOpacity
      style={[componentStyles.button, size, disabled && componentStyles.buttonDisabled, style]}
      onPress={onPress}
      disabled={disabled || loading}
      {...props}
    >
      <LinearGradient
        colors={gradient}
        style={[componentStyles.buttonGradient, size]}
        start={{ x: 0, y: 0 }}
        end={{ x: 1, y: 0 }}
      >
        {loading ? (
